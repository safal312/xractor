...
Xractor is a module that allows collection of [X-Ray data](https://www.amazon.com/salp/xray) from the Amazon Prime Video Platform. This tool allows a user to pass in a set of links for automatic collection for large scale analysis.
...
- **workers (int, optional, default: 1):**
  - Number of parallel workers for concurrent collection, improving performance.
...
- **base_url (str, optional, default: "https://www.amazon.com"):**
  - The base URL to start collection from. An example url in the input file needs to something like `/gp/video/detail/B0B683GB78/ref=atv_dp_atf_prime_sd_mv_resume_t1ADAAAAAA0wr0?autoplay=1&t=0`. The base_url is concatenated afterwards.

- **sign_in_url (str, optional, default: "https://www.amazon.com/gp/sign-in.html"):**
  - URL for signing in.

- **cookies_file (str, optional, default: "cookies.pkl"):**
  - File to store cookies for persistent sessions across multiple runs. If the file is not present at the start of the program, the user will be prompted to sign-in first.
...
### Recommendations
Xractor uses multiple heuristics to collect the data while navigating through the complex nature of the Amazon Prime Video platform and network issues. Therefore, it is recommended to run the browser in visual mode (headless=False) to inspect the situation at certain times.
